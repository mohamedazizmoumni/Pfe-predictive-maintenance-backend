    ...
    public class NlpAnalysisPublisher {
        ...
        @Async("taskExecutor")
        public void publishImageAnalysis(NlpImageAnalysisDTO response) {
            if (response == null) {
                return;
            }

            try {
                Map<String, Object> alertPayload = buildImagePayload(response, "NLP_IMAGE_ALERT");
                Map<String, Object> insightPayload = buildImagePayload(response, "MACHINE_INSIGHT");

                messagingTemplate.convertAndSend("/topic/nlp-alerts", alertPayload);
                messagingTemplate.convertAndSend("/topic/machine-insights", insightPayload);

                log.debug("Published NLP image analysis {} to realtime topics", response.getId());
            } catch (Exception ex) {
                log.error("Failed to publish NLP image analysis {}: {}", response.getId(), ex.getMessage(), ex);
            }
        }

        private Map<String, Object> buildImagePayload(NlpImageAnalysisDTO response, String eventType) {
            Map<String, Object> payload = new LinkedHashMap<>();
            payload.put("eventType", eventType);
            payload.put("analysisId", response.getId());
            payload.put("machineId", response.getMachineId());
            ...
            payload.put("attachmentId", response.getAttachmentId());
            payload.put("createdAt", response.getCreatedAt() == null ? LocalDateTime.now() : response.getCreatedAt());
            return payload;
        }
    }
